  ...
  if (mmt === 2 && pas === "147896325") {
    if (tm === timmerer && pas === "147896325") {
      ...
      if (mer === true) {
        ...
        spech.speak("hello sir, I am checking control service and system details, Enitializing data from youtue google maps mail you class room and meet data Enilized , we are online and ready");
        mer = false;
      ...
      tm = timmerer, 0, 0, windowWidth, windowHeight;
...
function showResult() {
  ...
  if (mmt === 2) {
    ...
    } else if (img.includes('news') && img.includes('city')) {
      notefi.play();
      img = img.replace('show', '');
      img = img.replace('news', '');
      img = img.replace('of', '');
      img = img.replace('city', '');
      img = img.replace(' ', '');
      img = img.replace('  ', '');
      img = img.replace('   ', '');
      img = img.replace(' ', '');

      spech.speak('ok sir I am showing news of city' + img);
      window.open("https://www.hindustantimes.com/cities/" + img + "-news");


    } else if (img.includes('news') && img.includes('india')) {
      notefi.play();
      spech.speak('ok sir I am showing news of India');
      window.open("https://www.hindustantimes.com/india-news");


    } else if (img.includes('welcome') && img.includes('guest')) {
      welcome.play();
    ...
    } if (img.includes('how are you')) {
      notefi.play();
      spech.speak('I am fine ,how are you');

    } else if (img.includes('camera')) {
      let capture;
  ...
  if (img.includes('unlock 2006') || img.includes('deactivate lockdown 2006') || img.includes('deactivate protocol 2911') || img.includes('deactivate lock down 2006') || img.includes('deactivate emergency protocol 2911') || img.includes('deactivate protocol 2911') && mmt === 1 && ghr === 1) {
    ...
    tm = lloc, 0, 0, windowWidth, windowHeight;
...
function getweather() {
  ...
  let url = 'http://api.weatherapi.com/v1/current.json?key= fb4dd9f4bb274d3db07145704210307&q=jammu';
  loadJSON(url, gotdatat);
...
function STD() {
  ...
  if (vavlus !== 0) {
    ...
    } else if (h === 11 && m === 6 && s === 0) {
      spech.speak("Sir this is time for your lunch break I recomend to take some meal to charge yourself for next classes ");

    } else if (h === 14 && m === 22 && s === 0) {
      spech.speak("Sir now your class is over you can see something for some time before lunch");
